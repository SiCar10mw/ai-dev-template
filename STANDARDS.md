...
| Centralized identity management: human and machine identities come from a central identity provider; machine identities are managed, scoped, conditional-access gated, audited, and short-lived. | `make setup-identity`, `scripts/setup_identity.sh`, `config/identity.example.json`, `docs/identity.md`; recommended default Microsoft Entra ID; alternatives include Okta, Google Cloud IAM / Workspace, and AWS IAM Identity Center |
...
| Central secrets manager: secrets come from a central secrets manager at runtime; tracked files and plain `.env` files contain only placeholder shape, never real values. | `make setup-identity`, `scripts/setup_identity.sh`, `SECRETS.md`, `config/secrets.example.json`, `ai_dev_template/secrets.py`; recommended default Azure Key Vault; alternatives include HashiCorp Vault, AWS Secrets Manager, GCP Secret Manager, 1Password/Doppler, and OS keyring for local only |
...
| GitHub operationalization is repo-scoped, dry-runable, idempotent, and human-gated before outward writes. | `scripts/operationalize.sh`, `docs/operationalize.md`, `tests/unit/test_operationalize.py`; refuses until `make setup-identity` records identity/secrets choices |
