...
| Least privilege and read-only-by-default for external systems; writes require explicit human approval. | `CONSTITUTION.md`, `.claude/agents/backlog-worker.md`, PR template language |
...
| Model/provider routing is explicit, secret-free, and subordinate to deterministic gates. | `config/model-routing.example.json`, `docs/model-routing.md`, Copilot CLI model selection |
...
| Machine-user pull request flow: bot may author PRs; human approves; bot cannot self-approve or self-merge. | `docs/machine-user-pr-flow.md`, `.claude/agents/backlog-worker.md` |
...
| Symphony backlog loop: human curates; worker claims one item, moves it through the board, and stops for approval. | `BACKLOG.md`, `.claude/agents/backlog-worker.md` |
| Core agent fireteam ships with every project. | `.claude/agents/*.md`, `docs/agent-fireteam.md` |
| Agents are approved-models-only, audit-logged, no-exfil, and sensitivity-aware. | `config/approved-models.example.json`, persona files, `docs/agent-fireteam.md` |
| Playbook -> weapon -> gate maturity: every capability climbs documented -> automated -> enforced. | `docs/agent-fireteam.md`, scripts, CI |
| Independent/adversarial review: a second AI attempts to refute the first AI's output; humans adjudicate disagreements. | `.claude/agents/independent-reviewer.md`, `docs/independent-review.md`, `review-prompt.md` |
